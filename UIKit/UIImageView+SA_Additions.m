...
//	float							aspect, myAspect, newWidth, newHeight;
	...
	if (self.contentMode == UIViewContentModeScaleAspectFill || self.contentMode == UIViewContentModeScaleToFill || self.contentMode == UIViewContentModeRedraw) return self.bounds;
	
	//if (size.width >= mySize.width && size.height >= mySize.height && self.contentMode != UIViewContentModeScaleAspectFit) return self.bounds;
	
	return CGRectPlacedInRectWithContentMode(CGRectFromSize(size), self.bounds, self.contentMode);
